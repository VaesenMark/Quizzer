...
import {
    editListSizeAction, editColorAction,
    closePrefsAction, closeAndApplyPrefsAction
} from '../reducers';
...
class PreferencesDialogUI extends React.Component {
    ...
    doCancelPrefs() {
        this.props.closePrefsScreen();
    }

    colorHandler(evt) {
        this.props.doEditColor(evt.target.value)
    }

    listSizeHandler(evt) {
        this.props.doEditListSize(evt.target.value)
    }
    ...
    render() {
        ...
        let colorOptions = ["green", "orange", "brown"].map(
        ...
        return (
            <div className='PreferencesDialog'>
                <div className={`bla ${this.props.color}-1`}></div>
                <header><Logo title="Settings"/></header>
                <label htmlFor="listSizeField">
                    Show <input type="number" id="listSizeField" value={this.props.listSize} onChange={this.listSizeHandler.bind(this)}/>
                    items in the list.
                </label>
                {warning}
                <label htmlFor="colorField">
                    color: <select id="colorField" value={this.props.color} onChange={this.colorHandler.bind(this)}>
                    {colorOptions}
                </select>
                </label>
                <div className="dialogButtons">
                    <button onClick={okHandler}>OK</button>
                    <button onClick={this.doCancelPrefs.bind(this)}>Cancel</button>
                </div>
            </div>
        );
    }
...
function mapStateToProps(state) {
    return {
        color: state.prefs.editingColor,
        listSize: state.prefs.editingListSize
    }
}
...
function mapDispatchToProps(dispatch) {
    return {
        closePrefsScreen: () => dispatch(closePrefsAction()),
        doEditColor: (selectedColor) => dispatch(editColorAction(selectedColor)),
        doCloseAndApplyPrefs: () => dispatch(closeAndApplyPrefsAction()),
        doEditListSize: (itemCount) => dispatch(editListSizeAction(itemCount))
    }
}
